  ...
  async shareContact(contact: Contact): Promise<void> {
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        Alert.alert("Share Contact", contactInfo, [
          { text: "Cancel", style: "cancel" },
          {
            text: "Copy to Clipboard",
            onPress: () => this.copyToClipboard(contactInfo),
          },
        ]);
      }
    } catch (error) {
  ...
  async addToPhoneContacts(contact: Contact): Promise<void> {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Required",
          "Please grant contacts permission to add this contact"
        );
        return;
      }
      ...
      const contactData: Contacts.Contact = {
        ...
        emails: contact.email
          ? [{ email: contact.email, isPrimary: true, label: "work" }]
          : undefined,
        phoneNumbers: contact.phone
          ? [{ number: contact.phone, isPrimary: true, label: "work" }]
          : undefined,
      ...
      if (contactId) {
        Alert.alert(
          "Success",
          `${contact.fullName} has been added to your contacts`
        );
      } else {
