...
const BASE_URL = 'https://hr.kms-technology.com/api';
...
class KMSAPIService {
  ...
  async fetchContacts(): Promise<APIResponse<Contact[]>> {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/Contact/ReturnContactList/0/0`, {
        headers: this.getHeaders(),
      });
      console.log('Contacts response:', response.data);
      if (response.data) {
        // Transform the API response to match our Contact interface
        const contacts: Contact[] = response.data.map((item: any) => ({
          id: item.id || item.employeeId || Math.random().toString(36).substr(2, 9),
  ...
  async updateContact(contactId: string, contactData: Partial<Contact>): Promise<APIResponse<Contact>> {
    try {
      if (!this.token) {
        return {
          success: false,
      ...
      const response = await axios.put(`${BASE_URL}/Contact/${contactId}`, contactData, {
        headers: this.getHeaders(),
      });
